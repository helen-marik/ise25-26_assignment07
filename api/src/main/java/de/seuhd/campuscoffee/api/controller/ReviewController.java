...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    public ResponseEntity<List<ReviewDto>> filter(
            @Parameter(description="Unique identifier of the POS to filter reviews for.", required=true)
            @RequestParam("pos_id") Long posId,
            @Parameter(description="Approval status to filter by.", required=true)
            @RequestParam("approved") Boolean approved) {
        return ResponseEntity.ok(
                reviewService.filter(posId, approved).stream() //filters posId and approved together
